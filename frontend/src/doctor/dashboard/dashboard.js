...
import { faBars, faTimes,faUser, faChartBar ,faSignOutAlt} from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className="flex">
      ...
      <div className={`fixed top-0 left-0 w-full flex items-center justify-between bg-green-600 py-2 px-4 ${open ? "ml-64" : ""}`}>
        ...
        <h6 className="text-white text-lg font-semibold">Medicare (Doctors)</h6>
      </div>
      ...
      <nav className={`bg-white w-64 min-h-screen shadow-lg ${open ? "translate-x-0" : "-translate-x-full"} fixed top-0 left-0 transition-all ease-in-out duration-300 z-50`}>
        ...
        <ul className="py-4 px-2">
          <li className="mb-2">
            <a href ="/doctor/dashboard" className="flex items-center text-gray-700 hover:text-green-600">
              <span className="mr-2">
          ...
          <li className="mb-2">
            <Link to="/complete-details" className="flex items-center text-gray-700 hover:text-green-600">
              <span className="mr-2">
                <FontAwesomeIcon icon={faUser} size="lg" />
              </span>
              Complete Profile
