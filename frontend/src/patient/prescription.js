...
const PrescriptionList = () => {
  ...
  <p className="mb-1"><span className="font-bold">Name:</span> HealthCare</p>
...
<div className='flex '>
    <div>
            <p className="text-lg font-semibold mb-2 text-gray-800">Patient: {prescription.patientName}</p>
            <p className="text-gray-600 mb-1">Date: {new Date(prescription.date).toLocaleDateString()}</p>
            <p className="text-gray-600 mb-1">Time: {prescription.time}</p>
            ...
            <ul className="ml-4 space-y-2">
              {prescription.medications.map((medication, medIndex) => (
                <li key={medIndex} className="bg-gray-100 p-2 rounded-md">
                  <p className="font-semibold text-gray-800 mb-1">Name: {medication.name}</p>
