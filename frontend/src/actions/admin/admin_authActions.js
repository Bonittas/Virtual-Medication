...
export const signIn = (email, password) => {
  return async (dispatch) => {
    try {
      const response = await axios.post("https://medicare-3.onrender.com/api/auth/admin/signin", { email, password });
...
export const logout = (navigate) => { // Accept navigate function as argument
  return async (dispatch) => {
    try {
      await axios.post("https://medicare-3.onrender.com/api/auth/admin/signout");
